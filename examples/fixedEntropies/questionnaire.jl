...
# Benchmark of different methods using NSB estimator for estimating entropy
for j in 2:4
    discrete = Matrix(df[:, 6:6+j-1])
    ...
    for i in 1:j-1
        println("Marginal size ", i)
        println("Raw polymatroid CSC")
        b = @benchmark max_ent_fixed_ent_unnormalized($distribution, $i, NsbPolymatroid())
        display(b)
        println("Raw polymatroid Mosek")
        b = @benchmark max_ent_fixed_ent_unnormalized($distribution, $i, NsbPolymatroid($false, $Mosek.Optimizer(), $0))
        display(b)
        if (j >= 4) 
            println("Raw polymatroid CSC Zhang-Yeung")
            b = @benchmark max_ent_fixed_ent_unnormalized($distribution, $i, NsbPolymatroid($true))
            display(b)
            println("Raw polymatroid Mosek Zhang-Yeung")
            b = @benchmark max_ent_fixed_ent_unnormalized($distribution, $i, NsbPolymatroid($true, $Mosek.Optimizer(),$0))
            display(b)
        end
    end
end
